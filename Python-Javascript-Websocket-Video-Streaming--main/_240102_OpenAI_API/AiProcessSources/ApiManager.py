...
"""
    ...
    def __IsValidDataSet__(self, dataSet : dict) :
        try :
            testValue : any = ""
            testValue = dataSet["messages"]
            
            if(len(testValue) != 3) : return False;
        
            systemSlot = testValue[0]
            userSlot = testValue[1]
            assistantSlot = testValue[2]
            
            
            if not isinstance(systemSlot["role"], str) : return False       
            if not isinstance(userSlot["role"], str) : return False       
            if not isinstance(assistantSlot["role"], str) : return False   

            if systemSlot["role"] != "system" : return False
            if userSlot["role"] != "user" : return False
            if assistantSlot["role"] != "assistant" : return False

            
            if not isinstance(systemSlot["content"], str) : return False       
            if not isinstance(userSlot["content"], str) : return False       
            if not isinstance(assistantSlot["content"], str) : return False            

            if systemSlot["content"] == "" : return False
            if userSlot["content"] == "" : return False
            if assistantSlot["content"] == "" : return False
    ...
    def InitDataSet(self, lectureName : str, filePath : str) : # callback = Action<string msg>
        ...
        self.dataSetBuffer : list[dict] = list[dict]()
        ...
        while len(thrList) > 0 :
            ...
            sleep(1)
            countdown -= 1;
            printProcess(rf"데이터 라벨링 스레드 {len(thrList)}개 남음 {countdown}초 남음!")

            if countdown == 0 :
                printWarning("180초 경과! 남은 데이터 라벨링 스레드들을 모두 파괴한다.")
                break;
        ...
        for dataSetList in self.dataSetBuffer :
            for dataSet in dataSetList :
                if self.__IsValidDataSet__(dataSet) :
                    retBuffer.append(dataSet)
                else :
                    printWarning(rf"유효하지 않은 데이터 구조 제거 : {dataSet}")
