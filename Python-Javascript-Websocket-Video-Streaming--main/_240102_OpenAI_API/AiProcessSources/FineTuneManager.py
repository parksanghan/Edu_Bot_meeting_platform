...
class FineTuneManager () :
    def __init__(self) :
        ...
        # lecture : str => filePaths : list[str]int
    ...
    def LecturesCreate(self, lecture : str) : 
        ...
        # 자료구조에 저장
        self.trainedTree[lecture] = [];
        self.trainingTree[lecture] = [];
        self.modelLib[lecture] = "";
        ...
        with open(rf"{lecPath}\lecture_info.json", "w", encoding = "utf-8") as json_file:
            json.dump(lectureData, json_file, ensure_ascii=False)
    ...
    def LecturesLoad(self) : 
        try :
            ...
            for lecture in directories:
                ...
                # lecture_info를 읽어오기
                with open(rf"{full_path}\lecture_info.json", "r", encoding="utf-8") as json_file:
    ...
    def LecturesSave(self) : 
        try :
            ...
            for lecture in directories:
                ...
                # lecture_info를 읽어오기
                with open(rf"{full_path}\lecture_info.json", "w", encoding = "utf-8") as json_file:
                    json.dump(lectureData, json_file, ensure_ascii=False)
                    
        except Exception as ex :
            printError("LecturesSave - 예외" + str(ex));
            printNor(rf" {lectureData}")
    ...
    def StartFineTuneJob(self, lectureName : str) :
        ...
        try :
            ...
            printProcess(f"'{lectureName}' 과목의 파인튜닝이 종료됐습니다.")
            ...
            printError(rf"StartFineTuneJob - 예외 : {ex}\n {ex.with_traceback.format_exc()}")
            printProcess(f"'{lectureName}' 과목의 파인튜닝이 종료됐습니다.")
            self.finetuneSteady[lectureName] = False;
    ...
    def __threadFineTuneJob__(self, lectureName : str, ftJobData : FineTuningJob,  callback) : 
        ...
        from _240102_OpenAI_API.AiProcessSources.ProcessManager import p1, p2
        while True :
            try :
    ...
    def FindSimilarity(self, lecture : str, question : str) :
        ...
        if ret != None and nowValue < 20 :
            printSucceed(rf"적합한 이미지를 찾았습니다! 유사도 : {nowValue} / 경로 : {ret}")
        
        elif nowValue >= 20: 
            printError(rf"이미지의 유사도가 낮아 유효하지 않습니다.")
            ret = None
        else :
    ...
    def __MakeDataSetWhole__(self, lecture : str) :
        ...
        printProcess("미처 완료되지 않은 로우데이터를 찾았습니다.")  
    ...
    # 캐시 생성
    def __CashCreate__(self, lecture) :
        try :
            lecPath = self.__lectureToPath__(lecture) + r"\cash_data.json";
        
            if os.path.exists(lecPath) :
                printWarning(rf"{lecture}과목의 캐시 메모리가 이미 존재합니다. 생성을 건너뜁니다.")
                return False
        
            emptyCash = {};
        
            with open(lecPath, "w", encoding = "utf-8") as json_file:
                json.dump(emptyCash, json_file, ensure_ascii=False)
            
            printSucceed(rf"{lecture}과목의 캐시 메모리를 성공적으로 생성했습니다.")
            return True
        
        except Exception as ex:
            printError(rf"{lecture}과목의 캐시 메모리 생성 중 예외 : {ex}")
            return False
    
    # 캐시 생성
    def __CashWipe__(self, lecture) :
        try :
            lecPath = self.__lectureToPath__(lecture) + r"\cash_data.json";
        
            if not os.path.exists(lecPath) :
                printWarning(rf"{lecture}과목의 캐시 메모리가 존재하지 않아 초기화를 취소합니다.")
                return False
            ...
            printSucceed(rf"{lecture}과목의 캐시 메모리를 성공적으로 초기화했습니다.")
            return True
        
        except Exception as ex:
            printError(rf"{lecture}과목의 캐시 메모리 초기화 중 예외 : {ex}")
            return False
        
    # 캐시 추가
    def __CashAdd__(self, lecture, question : str, answer : str, img : str) : 
        try : 
            lecPath = self.__lectureToPath__(lecture) + r"\cash_data.json";
            
            if not os.path.exists(lecPath) :
                printWarning(rf"{lecture}과목의 캐시 메모리가 존재하지 않습니다. 캐시 메모리를 생성합니다.")
                self.__CashCreate__(lecture)        

            data : dict = {}
            
            with open(lecPath, "r", encoding = "utf-8") as json_file:
                data = json.load(json_file)
                            
            data[question] = {"answer" : answer, "img" : img}
            
            with open(lecPath, "w", encoding = "utf-8") as json_file:
                json.dump(data, json_file, ensure_ascii=False)
                
            printSucceed(rf"{lecture}과목의 캐시 메모리가 성공적으로 저장됐습니다.\n[{question}] - [{answer}] - [{img}]")
            return True
        
        except Exception as ex:
            printError(rf"{lecture}과목의 캐시 메모리 추가 중 예외 : {ex}\n[{question}] - [{answer}] - [{img}]")
            return False
            
    # 캐시 참조
    def __CashGet__(self, lecture, question) :
        try : 
            lecPath = self.__lectureToPath__(lecture) + r"\cash_data.json";
            ...
            if question in data :
                printSucceed(rf"{lecture}과목의 캐시 참조가 성공했습니다.\n[{data[question]}]")
                return data[question];
            
            printWarning(rf"{lecture}과목의 캐시 참조가 실패했습니다.\n[None]")
            return None

        except Exception as ex:
            printError(rf"{lecture}과목의 {question}에 대한 캐시 참조 중 예외 : {ex}")
            return False
