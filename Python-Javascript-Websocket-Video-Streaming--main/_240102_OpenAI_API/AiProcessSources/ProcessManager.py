        ...
        try : 
            match flag: 
                ...
                case "GetAnswer": 
                    ...
                    #캐시 있는지 확인
                    cashResult = self.ft.__CashGet__(lecture, question);
                    
                    # 없다면 API 요청 및 이미지 유사도 검사 
                    if cashResult == None :               
                        def __callbackAnswerFun__(sid, lecture, answer) :
                            printSucceed(f"질문에 대한 답변 생성 ({sid}/{lecture}) \n[Quesetion] : {question} \n[Answer] : {answer}")
                            
                            cAns = answer;

                            self.client.Send(f"GetAnswerText{p1}{sid}{p2}{lecture}{p2}{answer}")                     
                            
                            # 캐시 저장
                            preData =  self.ft.__CashGet__(lecture, question)
                            if(preData == None) :
                                self.ft.__CashAdd__(lecture, question, cAns, "")
                            else :
                                self.ft.__CashAdd__(lecture, question, cAns, preData["img"])
                            

                        def __callbackImageFun__(sid, lecture, question) :
                            imgRoot : str = self.ft.FindSimilarity(lecture, question)                   
                            printSucceed(f"유사도 높은 이미지 ({sid}/{lecture}) \n[image] : {imgRoot}")
                            
                            cImg = imgRoot;
                    
                            self.client.Send(f"GetAnswerImage{p1}{sid}{p2}{lecture}{p2}{'None' if imgRoot == None else imgRoot}")  
                            
                            # 캐시 저장
                            preData =  self.ft.__CashGet__(lecture, question)
                            if(preData == None) :
                                self.ft.__CashAdd__(lecture, question, "", cImg)
                            else :
                                self.ft.__CashAdd__(lecture, question, preData["answer"], cImg)
                        
                        # 답변 스레드
                        self.api.InitRequest(sid, lecture, question, callback=__callbackAnswerFun__)
                        
                        #기다려!
                        while  self.ft.__CashGet__(lecture, question) == None : 
                            sleep(1); 
                        
                        # 이미지 스레드
                        imgThr : threading.Thread = threading.Thread(target = __callbackImageFun__, args= (sid, lecture, question)) 
                        imgThr.start();    
                    # 있다면 걔내 반환
                    else :
                        cAns = cashResult["answer"];
                        printSucceed(f"질문에 대한 답변 생성 ({sid}/{lecture}) \n[Quesetion] : {question} \n[Answer] : {cAns}")
                        self.client.Send(f"GetAnswerText{p1}{sid}{p2}{lecture}{p2}{cAns}")   
                        
                        cImg = cashResult["img"];
                        printSucceed(f"유사도 높은 이미지 ({sid}/{lecture}) \n[image] : {cImg}")
                        self.client.Send(f"GetAnswerImage{p1}{sid}{p2}{lecture}{p2}{'None' if cImg == None else cImg}")   
