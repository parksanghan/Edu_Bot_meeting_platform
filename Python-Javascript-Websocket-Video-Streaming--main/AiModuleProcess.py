...
async def send_file(filepath,target_sid):
    if os.path.exists(filepath):
        async with aiofiles.open(filepath,'rb') as file:
            ...
            await subsio.emit('GetAnswer_Add', data=target_file , to=target_sid)
            print("GetAnswer_Event called")
    
    else:
        printError(f"file is not exist.{filepath}")
        ...
        try : 
            match flag: 
                ...
                case "GetAnswerText": 
                    ...
                    print("GetAnswer emit succeed")
                case "GetAnswerImage": 
                    ssp = sp[1].split(p2)
                    sid = ssp[0]
                    lecture = ssp[1]
                    is_filepath_exist = ssp[2]
                    filepath= None
                    ...
                    if filepath is not None:
                        ...
                        asyncio.run(subsio.emit('GetAnswer_Add',data="None",to=sid))
                    printSucceed(f"['{sid}' 사용자의에게 이미지 전송 [image]{filepath}\n")
                    
                    print("GetAnswerImage emit succeed")
